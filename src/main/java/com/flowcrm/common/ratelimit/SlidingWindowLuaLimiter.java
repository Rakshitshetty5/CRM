...
@ConditionalOnProperty(name = "app.rate-limit.method", havingValue = "sliding-lua", matchIfMissing = true)
public class SlidingWindowLuaLimiter implements RateLimiter {
    ...
    @Override
    public RateLimiterResult check(String key, int maxRequests, long windowSeconds) {
        try {
            ...
            if (result == null || result.isEmpty()) {
                ...
                return RateLimiterResult.allowed(maxRequests);
            }
            ...
            if (!allowed) {
                int retryAfter = oldestScoreMs > 0
                        ...
                        : (int) windowSeconds;
                return RateLimiterResult.denied(retryAfter);
            }

            return RateLimiterResult.allowed(remaining);
